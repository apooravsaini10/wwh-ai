...
<div id="legend">
    <div class="legend-item">
        ...
        <span>WWH Identity</span>
    </div>
    <div class="legend-item">
        <div class="legend-dot" style="background:#50C878"></div>
        <span>Interaction</span>
    </div>
    <div class="legend-item">
        <div class="legend-dot" style="background:#C8A951"></div>
        <span>Learning</span>
    </div>
</div>
...
def memory_map():
    with driver.session() as session:
        ...
        result = session.run("""
            ...
            OPTIONAL MATCH (ai)-[:LEARNED]->(i:Interaction)
            OPTIONAL MATCH (ai)-[:KNOWS]->(l:Learning)
            RETURN ai, collect(distinct i) as interactions, collect(distinct l) as learnings
        """)
        ...
        nodes.append({
            ...
            "size": 40,
            "color": "#4A90D9",
            "info": "<b>IDENTITY</b><br><br>" +
                    "<b>WHAT:</b> " + ai_node.get('what', '') + "<br><br>" +
                    "<b>WHY:</b> " + ai_node.get('why', '') + "<br><br>" +
                    "<b>HOW:</b> " + ai_node.get('how', '')
        })
        ...
        for i, interaction in enumerate(interactions):
            if interaction:
                ...
                nodes.append({
                    ...
                    "size": 20,
                    "color": color,
                    "info": "<b>QUERY:</b> " + interaction.get('query', '') + "<br><br>" +
                            "<b>INTENT:</b> " + interaction.get('intent', '') + "<br><br>" +
                            "<b>SCORE:</b> " + str(score) + "<br><br>" +
                            "<b>TIME:</b> " + interaction.get('timestamp', '')
                })
        ...
        for i, learning in enumerate(learnings):
            if learning:
                ...
                nodes.append({
                    ...
                    "size": 15,
                    "color": "#C8A951",
                    "info": "<b>LEARNING:</b><br>" + learning.get('learning', '') + "<br><br>" +
                            "<b>TIME:</b> " + learning.get('timestamp', '')
                })
                edges.append({"from": 0, "to": node_id, "label": "KNOWS"})
        
        # Calculate stats
        total_interactions = len([i for i in interactions if i])
        total_learnings = len([l for l in learnings if l])
        
        scores = [i.get('validation_score', 0) for i in interactions if i]
...
def debug():
    with driver.session() as session:
        ...
        return "<br>".join(records) if records else "NO DATA IN DATABASE"
